...
var updateAverageRating = function(location) {
    if(location.reviews && location.reviews.length) {
        ...
        location.rating = parseInt(ratingTotal / reviewsCount, 10);
        location.save(function(err, location) {
            if(err) {
                console.log(err);
            } else {
                console.log('Rating updated to: ' + location.rating);
            }
        });
    }
};
...
module.exports.reviewsUpdateOne = function(req, res) {
    ...
    Loc.findById(req.params.locationid).select('rating reviews')
